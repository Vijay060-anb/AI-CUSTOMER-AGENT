...
def process_customer_message(
    ...
    if not order_id:
        previous_order_id = get_previous_order_id(
            conversation_id
        )
    ...
    # ========================================
    # 2. CONVERSATION-AWARE INTENT CLASSIFICATION
    # ========================================
    ...
    if history:
        ...
        for item in reversed(history):
            ...
            # Skip messages that contain only an order ID.
            if re.fullmatch(r"\s*\d{4,}\s*", candidate):
                continue

            previous_user_message = candidate
            break

        if previous_user_message:
            previous_intent = hybrid_classify(
                previous_user_message
            )["final_intent"]
    ...
    if order_id_only and history:
        ...
        if previous_user_message:
            ...
            routing = hybrid_classify(
                previous_user_message
            )

            routing["message"] = message
            routing["context_used"] = True
            routing["context_source"] = (
                "previous_user_message"
            )
    ...
    # If the previous conversation step was a change-order request
    # and the current message is not an order ID, treat it as the new item.
    if (
        previous_intent == "change_order"
        and not order_id_only
        and message.strip()
    ):
        routing = hybrid_classify(
            "I want to change my order to " + message
        )
        routing["message"] = message
        routing["context_used"] = True
        routing["context_source"] = "previous_change_order_intent"

    intent = routing["final_intent"]

    # Store the current user message AFTER
    # conversation context has been resolved.
    add_message(
        conversation_id,
        "user",
        message
    ...
    # Recover the new item during a multi-turn change-order conversation.
    if (
        intent == "change_order"
        and not new_item
        and previous_intent == "change_order"
        and not order_id_only
    ):
        new_item = message.strip()
    ...
    if intent == "track_order":
        ...
        customer = customer_name if customer_name else "Guest Customer"
